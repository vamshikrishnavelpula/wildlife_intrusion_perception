...
# --- Confidence thresholds per animal ---
...
try:
    while True:
        ...
        # Check for target animal sounds with per-animal threshold
        for idx in top5:
            ...
            if label in reverse_map:
                ...
                threshold = confidence_thresholds.get(animal, 0.15)
                if conf > threshold:
                    detected = True
                    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    print(f"🐾 [{timestamp}] DETECTED: {animal.upper()} ({conf*100:.1f}%) [threshold={threshold*100:.0f}%]")
